...
const ModalRiscosPGR = ({ isOpen, onClose, onSelect }) => {
    ...
    const fetchRiscos = async () => {
        ...
        setLoading(true);
        try {
            const response = await axios.get('/riscos-catalogo', {
                params: {
                    page: currentPage - 1,
                    size: itemsPerPage,
                    search: searchTerm
            ...
            setRiscos(response.data.content || []);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl overflow-hidden">
                <div className="flex justify-between items-center px-6 py-4 bg-gray-50 border-b">
                    <h3 className="text-lg font-semibold text-gray-800">Selecionar Riscos Trabalhistas (PGR)</h3>
                        ...
                        <>
                            <div className="overflow-x-auto max-h-96">
                                <table className="min-w-full bg-white border border-gray-200">
                                    ...
                                    <tr>
                                        <th className="text-left py-3 px-4 font-semibold text-gray-600 text-sm">Grupo</th>
                                    ...
                                    <tbody>
                                    {riscos.map((risco) => (
                                        <tr key={risco.id} className="border-b border-gray-200 hover:bg-gray-50">
                                            <td className="py-3 px-4 text-sm text-gray-700">{risco.grupo}</td>
                                            <td className="py-3 px-4 text-sm text-gray-700">{risco.descricao}</td>
