...
export default function ListarEpiEpc() {
    const [searchTerm, setSearchTerm] = useState('');
    const [currentPage, setCurrentPage] = useState(1);
    const [entriesPerPage, setEntriesPerPage] = useState(5);

    const filteredData = epiData.filter(item =>
        ...
        <div className="bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8 font-sans">
            <div className="container mx-auto">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                    <h1 className="text-3xl font-bold text-gray-900 mb-4 sm:mb-0">EPI/EPC</h1>
                    <div className="flex flex-wrap gap-2">
                        <Link
                            to="/cadastros/novo-epi"
                ...
                <div className="bg-white p-4 rounded-lg shadow-md">
                    {/* Filtro por Empresa */}
                    <div className="mb-4">
                        <label className="text-sm font-medium text-gray-600">Filtrar equipamentos por empresa</label>
                        <InputWithActions
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            {currentEntries.map((item, index) => (
                                <tr key={index} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.tipo}</td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.fabricante || '--'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.validade || '--'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.criadoPor}</td>
