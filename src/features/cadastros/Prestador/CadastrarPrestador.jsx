...
export default function CadastrarPrestador({ isModal = false, onClose = () => {}, prestadorData = null }) {
    const { id: routeId } = useParams();
    const id = routeId || (prestadorData ? String(prestadorData.id) : null);
    ...
    useEffect(() => {
        if (isEditing && prestadorData) {
            const data = prestadorData;

            // Preenche o formulário com os dados recebidos na prop
            setFormData(prev => ({
                ...prev,
                ...data,
                cboId: data.cboId, // Garante que o cboId está no formData
                cbo: { id: data.cboId, descricao: data.cboNomeOcupacao }, // Preenche o objeto cbo
                endereco: data.endereco || prev.endereco,
            }));

            // Aplica as máscaras nos campos corretos
            setCpfMasked(applyCpfMask(data.cpf || ''));
            setTelefone1Masked(applyPhoneMask(data.telefone1 || ''));
            setTelefone2Masked(applyPhoneMask(data.telefone2 || ''));
        }
    }, [isEditing, prestadorData, setCpfMasked, setTelefone1Masked, setTelefone2Masked]);


    const handleInputChange = (e) => {
        const { name, value } = e.target;

        // Define qual máscara usar com base no nome do campo
        const maskType = name === 'cpf' ? 'cpf' : (name.startsWith('telefone') ? 'telefone' : null);

        // Aplica a máscara se for um campo correspondente
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                ...
                await prestadorServicoService.create(payload);
            }
