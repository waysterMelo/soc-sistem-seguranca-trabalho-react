...
import { prestadorServicoService } from '../../../api/services/cadastros/prestadorServicoService';
import EditarPrestadorModal from '../../cadastros/Prestador/EditarPrestadorModal.jsx';
...
export default function ListarPrestadores() {
    ...
    const fetchPrestadores = async () => {
        try {
            const res = await prestadorServicoService.getAll(0, 100);
            setPrestadores(res.data.content || res.data);
        } catch {
            toast.error("Erro ao carregar prestadores.");
        }
    };

    const filteredData = (Array.isArray(prestadores) ? prestadores : prestadores.content || [])
    ...
    const handleDelete = async (id) => {
        const confirm = window.confirm("Tem certeza que deseja excluir este prestador?");
        if (!confirm) return;
        try {
            await prestadorServicoService.delete(id);
            toast.success("Prestador excluído com sucesso!");
            fetchPrestadores();
        } catch (error) {
            if (error.response?.status === 409) {
                toast.error("Este prestador está vinculado a registros e não pode ser excluído.");
            } else {
    ...
    return (
        <div className="bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8 font-sans">
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            {currentEntries.map((prestador) => (
                                <tr key={prestador.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{prestador.cboNomeOcupacao || '-'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{prestador.cpf}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm"><StatusBadge status={prestador.status ? 'ATIVO' : 'INATIVO'} /></td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                                        <div className="flex items-center space-x-3">
                                            <Link
                                                to={`/cadastros/editar-prestador/${prestador.id}`}
            ...
            <EditarPrestadorModal
                ...
                prestadorId={editingPrestadorId}
