...
const FormSection = ({ title, children, className }) => (
  <div className={`bg-white p-6 rounded-lg shadow-md ${className}`}>
    {title && (
      <h3 className="text-xl font-semibold text-gray-800 border-b border-gray-200 pb-4 mb-6">
        {title}
      </h3>
    )}
  ...
  return (
    <div className="border border-gray-300 rounded-lg">
      ...
      <div
        ...
        className={`w-full p-4 ${heightClass} overflow-y-auto focus:outline-none rounded-b-lg ${
  ...
  const laudoSubTabs = [
    {
      ...
      field: "laudoResponsabilidadeTecnica",
    },
    ...
    { id: "objetivos", label: "Objetivos", field: "laudoObjetivos" },
    {
      ...
      field: "laudoConsideracoesGerais",
    },
    {
      id: "criterios",
      label: "Critérios de Avaliação",
      field: "laudoCriteriosAvaliacao",
    },
  ];
...
<p>Ressalta-se a <strong>necessidade de atualização periódica do laudo</strong>, especialmente em situações de alteração de processos, ambientes ou organização do trabalho, garantindo assim a efetividade do gerenciamento de riscos e a preservação da saúde dos trabalhadores.</p>
  ...
  const validarFormulario = () => {
    ...
    if (selectedProfissionais.length === 0)
      erros.push("Selecione pelo menos um profissional.");
    if (selectedSetores.length === 0)
      erros.push("Selecione pelo menos um setor.");
    if (!ltcatData.dataDocumento)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const fonteDeFuncoes = id
        ? funcoesDoLTCAT
        : selectedSetores.flatMap((setor) => setor.funcoes || []);
      ...
      toast.error(
        "Erro ao salvar LTCAT. Verifique os dados e tente novamente."
      );
    }
  };
  ...
  const handleSelectUnidade = (unidade) => {
    ...
    setLtcatData((p) => ({
      ...p,
      unidadeOperacionalId: unidade.unidadeOperacionalId,
  ...
  const handleSelectPrestador = (prestador) => {
    if (!selectedProfissionais.some((p) => p.id === prestador.id))
      setSelectedProfissionais((prev) => [...prev, prestador]);
    setIsPrestadorModalOpen(false);
  };
  const handleRemovePrestador = (prestadorId) =>
    setSelectedProfissionais((prev) =>
      prev.filter((p) => p.id !== prestadorId)
    );
  ...
  const handleSelectSetor = async (setor) => {
    ...
    const loadingToast = toast.loading(
      `Carregando detalhes do setor: ${setor.nome}...`
    );

    try {
      const setoresResponse = await setorService.getSetoresByEmpresa(
        selectedEmpresa.id,
        { page: 0, size: 1000 }
      );
  ...
  const handleRemoveSetor = (setorId) =>
    setSelectedSetores((prev) => prev.filter((s) => s.id !== setorId));
  ...
  const TabProfissionais = () => (
    <div className="space-y-4">
      <div>
        ...
        <InputWithActions
          placeholder="Clique para buscar um profissional"
          actions={
            <button
              type="button"
              onClick={() => setIsPrestadorModalOpen(true)}
      ...
      <div>
        ...
        <div className="mt-2 border rounded-md p-2 space-y-2 min-h-[50px]">
          {selectedProfissionais.map((prof) => (
            <div
              key={prof.id}
            ...
            >
              ...
              <button
                type="button"
                onClick={() => handleRemovePrestador(prof.id)}
  ...
  const TabAparelhagem = () => (
    <div className="space-y-4">
      <div className="flex justify-end">
        <button
          type="button"
          onClick={() => setIsAparelhagemModalOpen(true)}
          className="flex items-center gap-2 text-sm bg-blue-600 text-white px-3 py-2 rounded-md hover:bg-blue-700"
        >
      ...
      <div className="mt-2 border rounded-md p-2 space-y-2 min-h-[50px]">
        ...
        {selectedAparelhos.map((aparelho) => (
          ...
          >
            ...
            <button
              type="button"
              onClick={() => handleRemoveAparelho(aparelho.id)}
              className="text-red-500 hover:text-red-700"
            >
              <Trash2 size={16} />
            </button>
  ...
  const mainTabs = [
    {
      ...
      component: (
        <TabCapa
          onFileChange={handleCapaFileChange}
          onRemove={handleRemoveCapaFile}
    ...
    },
    {
      ...
      component: (
        <RichTextEditor
          ...
          onChange={(html) =>
            setLtcatData((p) => ({ ...p, recomendacoesTecnicas: html }))
          }
        />
      ),
    },
      ...
      <div className="container mx-auto">
        <header className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">
            {id ? `Editar LTCAT #${id}` : "Cadastrar LTCAT"}
          </h1>
        </header>
        <form onSubmit={handleSubmit}>
          <FormSection title="Informações Básicas">
            ...
            <div className="col-span-2 flex items-end gap-2">
              <button
                type="button"
                className="px-3 py-2 text-sm bg-green-100 text-green-800 rounded-full"
              >
                Empresa
              </button>
              <button
            ...
            {id && (
              <div>
                ...
                <input
                  type="date"
                  name="dataVencimento"
                  value={ltcatData.dataVencimento}
                  onChange={handleInputChange}
                  className="mt-1 w-full py-2 px-3 border border-gray-300 rounded-md"
                />
              </div>
            ...
            <div>
              ...
              <input
                ...
                value={ltcatData.alertaValidadeDias}
                onChange={handleInputChange}
                className="mt-1 w-full py-2 px-3 border border-gray-300 rounded-md"
              />
            </div>
            ...
            <div className="col-span-full">
              <label className="text-sm font-medium text-gray-600">
                Comentários (não será impresso no LTCAT)
              </label>
              <input
                type="text"
            ...
            <div className="col-span-full">
              ...
              <RichTextEditor
                ...
                onChange={(html) =>
                  setLtcatData((p) => ({ ...p, condicoesPreliminares: html }))
                }
                heightClass="h-40"
              />
            </div>
